...
struct prf_info_s {
	bool black_key; /* true/false if i/p or o/p is
			   in black(encrypted) form or plain data */
	u16 len;
	void *param;
};

struct gen_master_secrete_s {
	struct prf_info_s label;
	struct prf_info_s pre_master_secret;
	struct prf_info_s server_rand;
	struct prf_info_s client_rand;
	struct prf_info_s out_master_secret;
};

struct gen_session_keys_s {
	struct prf_info_s label;
	struct prf_info_s master_secret;
	struct prf_info_s server_rand;
	struct prf_info_s client_rand;
	struct prf_info_s out_client_mac_secret;
	struct prf_info_s out_server_mac_secret;
	struct prf_info_s out_client_write_key;
	struct prf_info_s out_server_write_key;
...
struct prf_req_s {
	u32 prf_op;
	u32 tls_version; /* define copied from openssl/tls1.h header file
				#define TLS1_2_VERSION                  0x0303
				#define TLS1_1_VERSION                  0x0302
				#define TLS1_VERSION                    0x0301
			  */
	union {
		struct gen_master_secrete_s gen_ms;
